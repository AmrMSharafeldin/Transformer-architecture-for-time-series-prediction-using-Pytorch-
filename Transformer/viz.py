...
saveplotpath = Parameters.get("visualization_save_path")
    ...
    try:
        with data_zip.open(random_csv_file) as f:
            ...
            if test_data_csv.shape[0] > 15:
                ...
                y, p = evalY_tensor.cpu(), predictions.cpu()
                plot_test, plot_preds = y[1:], p[1:]
                df = test_data_csv['DlyPrc']
                x = df[-(plot_test.shape[0] * plot_test.shape[1]):].index
                plot_test, plot_preds = plot_test.reshape((plot_test.shape[0] * plot_test.shape[1], 1)), plot_preds.reshape((plot_test.shape[0] * plot_test.shape[1], 1))
                fig = plot_predictions(np.squeeze(plot_test.numpy()), np.squeeze(plot_preds.numpy()), x, True)
                fig.savefig(saveplotpath)
